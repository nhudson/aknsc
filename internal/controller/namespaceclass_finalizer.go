...
func (r *NamespaceClassReconciler) reconcileNamespaceClassFinalizer(ctx context.Context, nc *v1alpha1.NamespaceClass, logger logr.Logger) (ctrl.Result, error) {
	if nc.DeletionTimestamp.IsZero() {
		// Resource is NOT being deleted: ensure finalizer is present
		return r.ensureFinalizerPresent(ctx, nc, logger)
	} else {
		// Resource IS being deleted: handle cleanup and remove finalizer
		return r.handleFinalizerCleanup(ctx, nc, logger)
	}
}

// ensureFinalizerPresent adds the NamespaceClassFinalizer to the resource if not already present.
func (r *NamespaceClassReconciler) ensureFinalizerPresent(ctx context.Context, nc *v1alpha1.NamespaceClass, logger logr.Logger) (ctrl.Result, error) {
	if slices.Contains(nc.Finalizers, NamespaceClassFinalizer) {
...
func (r *NamespaceClassReconciler) handleFinalizerCleanup(ctx context.Context, nc *v1alpha1.NamespaceClass, logger logr.Logger) (ctrl.Result, error) {
	var errs []error
	...
	if err := r.List(ctx, nsList, client.MatchingLabels{NamespaceClassLabel: nc.Name}); err != nil {
		logger.Error(err, "Failed to list namespaces for NamespaceClass cleanup", "class", nc.Name)
		errs = append(errs, err)
	} else {
		for _, ns := range nsList.Items {
			// Skip cleanup for namespaces that are being deleted, as the resources will be garbage collected
			if ns.DeletionTimestamp != nil {
				logger.Info("Skipping cleanup for namespace that is already being deleted",
					"namespace", ns.Name,
					"class", nc.Name)
				continue
			}
			...
			for _, resource := range resources {
				...
				logger.Info("Deleting resource during NamespaceClass cleanup",
					"namespace", ns.Name,
					"kind", kind,
					"name", name)
			...
			if err := r.Get(ctx, types.NamespacedName{Name: ns.Name}, nsToUpdate); err != nil {
				logger.Error(err, "Failed to get namespace for annotation cleanup", "namespace", ns.Name)
				errs = append(errs, err)
				continue
			}
			...
			if nsToUpdate.Annotations != nil {
				...
				if err := r.Update(ctx, nsToUpdate); err != nil {
					logger.Error(err, "Failed to update namespace annotations during cleanup", "namespace", ns.Name)
					errs = append(errs, err)
				}
			}
		}
	...
	index := slices.Index(nc.Finalizers, NamespaceClassFinalizer)
	if index != -1 {
		logger.Info("Removing finalizer from NamespaceClass", "class", nc.Name)
		nc.Finalizers = slices.Delete(nc.Finalizers, index, index+1)
	...
	if hasNamespaceClassFinalizer {
		logger.Info("Removing finalizer from NamespaceClass", "class", nc.Name)
		if err := r.Update(ctx, nc); err != nil {
			logger.Error(err, "Failed to remove finalizer from NamespaceClass", "class", nc.Name)
			errs = append(errs, err)
		}
	}
	...
	logger.Info("Successfully cleaned up resources for NamespaceClass", "class", nc.Name)
	return ctrl.Result{}, nil
}
...
func (r *NamespaceClassReconciler) listResourcesOfType(
	...
	err := r.List(ctx, listObj, client.InNamespace(namespace), labelSelector)
	if err != nil {
